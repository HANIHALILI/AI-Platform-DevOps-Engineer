...
Ollama, and the Agent. Each watches `main`, syncs automatically, and prunes and
self-heals, so a change to a chart or its values reaches the cluster on its own
and a manual `kubectl edit` is undone.
