#!/usr/bin/env bash
# Rubric-based quality checks against the deployed model. Every case lists the
# concepts a grounded answer has to cover; the answers are kept for reading.
set -euo pipefail

NS=${NAMESPACE:-ai-platform}
CASES=${CASES:-benchmarks/quality/cases.psv}
